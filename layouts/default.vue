  ...
  <div class="app-shell">
    <header class="top-nav">
      <div class="brand-block">
        ...
        <client-only>
          <span v-if="showNav">{{ user && (user.displayName || user.email) }}</span>
        </client-only>
      </div>
      <template v-if="showNav">
        <nav class="nav-links" :key="$i18n.locale">
          <nuxt-link
            v-for="item in navItems"
            :key="item.to"
            :to="item.to"
            :class="item.className"
          >{{ $t(item.labelKey) }}</nuxt-link>
        </nav>
        <LanguageSwitcher />
...
export default {
  ...
  computed: {
    ...
    showNav() {
      return this.isAuthenticated || this.$route.path !== '/login'
    },

    navItems() {
      const userRole = this.user?.role || 'advisor'
...
.nav-home.nuxt-link-exact-active { background: linear-gradient(120deg, #ffd4b8, #ffc49e); border-color: rgba(242, 98, 0, 0.6); color: #a34200; }
...
.nav-dashboard.nuxt-link-exact-active { background: linear-gradient(120deg, #99e5f7, #66d9f2); border-color: rgba(0, 177, 224, 0.6); color: #007a99; }
...
.nav-pipeline.nuxt-link-exact-active { background: linear-gradient(120deg, #99b8db, #6695c9); border-color: rgba(0, 43, 100, 0.6); color: #001a3d; }
...
.nav-team.nuxt-link-exact-active { background: linear-gradient(120deg, #b3e8f5, #8cddf0); border-color: rgba(127, 211, 241, 0.7); color: #1e7a9e; }
...
.nav-coi.nuxt-link-exact-active { background: linear-gradient(120deg, #99dba8, #66cc7a); border-color: rgba(31, 157, 64, 0.6); color: #156d2d; }
...
.nav-blog.nuxt-link-exact-active { background: linear-gradient(120deg, #ee99ee, #e566e5); border-color: rgba(208, 21, 213, 0.6); color: #8a0e8e; }
...
.nav-lists.nuxt-link-exact-active { background: linear-gradient(120deg, #a5f3fc, #67e8f9); border-color: rgba(6, 182, 212, 0.6); color: #0891b2; }
